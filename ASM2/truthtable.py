...
class TruthTable:
    ...
    def generate_table(self):
        for combination in product([True, False], repeat=len(self.symbols)):
            model = {symbol: value for symbol, value in zip(self.symbols, combination)}
            yield (model, [self.knowledgeBase.evaluate(model)])

    def check_facts(self):
        for model, evaluation in self.generate_table():
            if all(evaluation) and self.query.evaluate(model):
                self.count += 1
        return False
    ...
    def __str__(self):
        rows = []
        for model, evaluations in self.generate_table():
            row = [str(model[symbol]) for symbol in self.symbols]
            row += [str(evaluations[0])] + [str(self.query.evaluate(model))]
            rows.append(row)
